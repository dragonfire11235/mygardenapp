...
**Nächster Plan (17.07., freigegeben, noch nicht begonnen):** `PLAN-profil-saas.md` — **lokales Nutzer-Profil + SaaS-Roadmap**. Teil 1 (jetzt bauen): Profil-Felder (Name/Gartenname/Profilfoto) im Settings-Store, Foto-GC-Schutz für `profilePhotoId`, Profil- + Konto-Karte in den Einstellungen, Name in der Dashboard-Begrüßung. Teil 2 (Referenz): Weg zum bezahlten SaaS (Edge-Function-Proxy zuerst → Auth E-Mail+Passwort → LWW-Sync → Stripe → Domain → Foto-Sync zuletzt) inkl. Liste, was der Nutzer selbst besorgen muss (Supabase EU-Region, Stripe, Domain, Rechtstexte, Steuerberater/EU-OSS). Nutzer recherchiert derzeit noch.
...
- **Gardena smart system** — Entschieden: Proxy nötig (CORS + Secret → kein Direktzugriff aus dem Browser); laut `PLAN-profil-saas.md` künftig als Supabase Edge Function statt Cloudflare Worker. Nötig: Application Key + Secret vom Husqvarna-Dev-Portal (beide APIs verbunden). `GardenaAdapter` an der bestehenden `DeviceAdapter`-Naht, `AdapterId 'gardena'`.
- **Home Assistant** (Adapter-Naht bereit), **Supabase-Sync** (siehe `PLAN-profil-saas.md` Teil 2 + Alt-Plan `PLAN-handy-supabase.md` Block B), Aussaat-/Ernte-Kalender, Ernte-Tracking, Perenual als 2. Datenquelle.
